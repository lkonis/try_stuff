...
subt <- function(t, pivot=1)
{
  ...
  if (pivot=="m")
  {
    # if t length is even
    if ((length(t) %% 2) == 0)
      mt = c(t[1],t[floor(length(t)/2)], t[length(t)])
    else
      mt = c(t[1],t[ceiling(length(t)/2)], t[length(t)])
    
    piv = which (t==median(mt))
  }
  else
  {
    piv = pivot
  ...
  else
  {
    ...
    # call recursion for left
#    cmp_n <<- cmp_n + (length(t_small) - 1)
    t_small = subt(t_small, pivot)
  }
  ...
  else
  {
    ...
    # call recursion for right
 #  cmp_n <<- cmp_n + (length(t_greater) - 1)
    t_greater = subt(t_greater,pivot)
  }
